    ...
    @foreach ($transaction_histories as $transaction_history)
        ...
        <tr class="recharge">
            ...
            <td class="list-table-time">
                <a href="javascript:void(0);">{{ date('d/m/Y', strtotime($transaction_history->created_at)) }}</a> 
                <a href="javascript:void(0);"><span>{{ date('H:i', strtotime($transaction_history->created_at)) }}</span></a>
            </td>
            <td class="list-table-so-tien {{$text_color}}">
                <a href="javascript:void(0)">{!! $transaction_history->type == 'deposit'?'+':'-'; !!} {{ formatCurrency($transaction_history->amount) }} VND</a>
            </td>
                    ...
                    <!-- Deposit Amount Options -->
                        <div class="depositAmount mb-4">
                            ...
                            <div class="depositAmount-Row">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="depositAmount" id="depositAmountOther" value="other">
                                    <label class="form-check-label" for="depositAmountOther"> Khác </label>
                                </div>
                            </div>
                            <input type="text" class="form-control mt-3" name="depositAmountCustom" id="depositAmountCustom" placeholder="Số tiền khác" style="display:none;">
                        </div>
                        <div class="mb-4 ">
