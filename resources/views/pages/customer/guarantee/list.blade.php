    ...
    <div class="row align-items-center">
        <div class="col-xl-10 col-md-8 col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active fw-700" aria-current="page">Yêu cầu bảo hành</li>
                </ol>
            </nav>
        </div>
...
<section class="section danh-sach-du-an mb-5">
    <div class="container-fluid">
        <div class="col-inner">
            <h2 class="section-title mb-4">Yêu cầu bảo hành</h2>
            ...
            <div class="group-table-list">
                <table class="table list-table">
                    ...
                    <tbody>
                        @if(!empty($support_guarantees))
                        @foreach ($support_guarantees as $key => $guarantee)
                        @php
                            $link_support = (!empty($guarantee->status) && $guarantee->status != 4) ? route('admin.reply.support', ['id' => $guarantee->id]) : route('customer.support.detail', ['id' => $guarantee->id]);
                        @endphp
                        <tr>
                            ...
                            <td class="list-table-title text-start" style="min-width: unset !important; width: 220px !important">
                                <a href="{{ $link_support }}" style="min-width: unset !important; width: 180px !important">{{ $guarantee->title }}</a>
                            </td>
                            <td class="list-table-sku text-start" style="min-width: unset; width: 150px !important">
                                <a href="javascript:void(0);" style="min-width: unset !important; width: 150px !important; text-align: left">{{ $guarantee->content }}</a>
                            </td>
                            <td class="text-center">{{ $guarantee->sender?->name ?? '' }}</td>
                            <td class="list-table-time text-center">
                                {!! date('d/m/Y h:i', strtotime($guarantee->created_at)) !!}
                            </td>
                            <td class="list-table-progree text-center">
                                <span class="btn {!! 
                                    $guarantee->status == 1 ? 'btn-success' : 
                                    ($guarantee->status == 2 ? 'btn-primary' :
                                    ($guarantee->status == 3 ? 'btn-info' : 
                                    ($guarantee->status == 4 ? 'btn-danger' : 'btn-danger')))
                                !!} ">{!! __( config('constants.status_support')[$guarantee->status]) !!}</span>
                            </td>
                            <td style="min-width: 45px; max-width: 45px; width: 45px">
                                @if(!empty($guarantee->status) && $guarantee->status != 4)
            ...
            @if(!empty($support_guarantees))
                {{ $support_guarantees->links('vendor.pagination.custom') }}
            @endif
        </div>
    </div>
</section>
